...
export function MetaBar({ items = [], style }: MetaBarProps) {
  return (
    <div style={{ width: "100%", borderRadius: 16, boxShadow: "var(--pf-ring)", display: "flex", flexDirection: "row",
      justifyContent: "space-between", alignItems: "center", overflow: "hidden", ...style }}>
      {items.map((it, i) => (
        <div key={i} style={{ flex: 1, minWidth: 0, border: "1px solid var(--pf-border)", boxSizing: "border-box",
