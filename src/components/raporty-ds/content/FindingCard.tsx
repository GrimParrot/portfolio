...
export function FindingCard({ number, title, children, style }: FindingCardProps) {
  return (
    <div style={{ borderRadius: 24, background: "var(--pf-surface-dark-card)", boxShadow: "var(--pf-ring-dark)",
      padding: "clamp(20px, 3vw, 32px)", display: "flex", flexDirection: "column", gap: 32, boxSizing: "border-box", flex: 1, minWidth: 0, ...style }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
        <span style={{ opacity: 0.3, fontFamily: "var(--pf-font-display)", fontWeight: 700, fontSize: "clamp(40px, 8vw, 80px)", lineHeight: "clamp(48px, 9vw, 108px)",
          letterSpacing: "0.02em", color: "var(--pf-primary-700)" }}>{number}</span>
        <span style={{ position: "relative", zIndex: 1, width: "100%", marginTop: "clamp(-30px, -6vw, -50px)", fontFamily: "var(--pf-font-body)", fontWeight: 700, fontSize: 22, lineHeight: "34px",
