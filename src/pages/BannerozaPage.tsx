...
function AutoFitHeading({ variants, activeKey, className, style, maxLines = 3, minFontPx = 26, heading = true }: { variants: { key: string; content: ReactNode }[]; activeKey: string; className?: string; style?: React.CSSProperties; maxLines?: number; minFontPx?: number; heading?: boolean }) {
  ...
  useLayoutEffect(() => {
    ...
    // A window "resize" event never fires when this heading is mounted inside
    // the project modal: the modal grows from the clicked card's small size to
    // full size via a Framer Motion layoutId transition (not a viewport
    // change), so fit() would otherwise measure and lock in the card's tiny
    // starting width. ResizeObserver catches that layout-driven width change too.
