...
Sekcja produktu to `.pf-product-row` — kolumna z odstępem 24px: temat sekcji (`.pf-product-label` + `.pf-overline`) nad kartą, karta na pełną szerokość. Cała strona jest przez to **jednokolumnowa**: hero, nagłówek „Products", każda etykieta i każda karta mają tę samą lewą krawędź i tę samą szerokość. Wcześniej numer stał w stałej kolumnie 180px obok karty, przez co karty były węższe niż wszystko nad nimi. Produkt **nie ma własnej ramki** — żadnego obramowania, tła, radiusa ani paddingu. Rozdziela je hairline między sobą, a nie pudełko dookoła każdego. Wewnątrz `gap: 48` i **pięć osobnych sekcji**: nazwa z tagami, opis, zakres (h4 `.pf-h4` + `.pf-role-cols`), rząd obrazków, podpis `.pf-caption` w `--pf-text-muted`. Opis jest osobną sekcją, nie doklejką do bloku nazwy — wcześniej siedział z nazwą i tagami w jednym bloku po 16px i wszystko się zlewało.

Pierwsza sekcja to h3 `.pf-h4` i chipy, 16px od siebie, a **znacznik stoi w jednej linii na lewo od h3**, nie nad nim: wiersz `flex`, `gap: 16`, znacznik 32×32 (radius 8, tło `--pf-accent-500`, ikona lucide 19px `strokeWidth 1.75` na biało). `align-items: flex-start`, nie `center` — na telefonie tytuł łamie się na trzy linie i wyśrodkowany znacznik pływałby w jego połowie; przy szerokościach desktopowych, gdzie tytuły są jednoliniowe, różnica to 2px.

Separator między produktami to `Divider` z `raporty-ds`, opakowany w `Section`. To opakowanie jest konieczne: `Divider` sam z siebie ma `maxWidth: 1200`, a kolumna treści ma 1152 (1200 minus padding `Section`), więc bez tego wystawałby po 24px z każdej strony i psuł jednokolumnowe wyrównanie. Separatory idą **między** produktami — nie przed pierwszym ani po ostatnim. Odstępy dokoła bierze wrapper strony (120px na desktopie), tak samo jak w Client Acquisition.

Chipy w karcie to **ten sam `Badge`, co wszędzie indziej** — `variant="secondary"` z `px-3 py-1.5 text-sm font-medium`, dokładnie jak tagi w hero każdej strony projektu (Raporty, Client Acquisition, PlanujemyTo, Banneroza, SimpleProjectPage). Figma rysuje tu szerszą pigułkę na ciemniejszej szarości; świadomie tego nie odwzorowujemy, bo byłby to jedyny zestaw tagów w serwisie wyglądający inaczej niż reszta. **Nie rób dla nich osobnego komponentu ani osobnych klas.**
