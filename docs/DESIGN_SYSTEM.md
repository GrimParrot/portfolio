...
Sekcja produktu to `.pf-product-row` — kolumna z odstępem 24px: temat sekcji (`.pf-product-label` + `.pf-overline`) nad kartą, karta na pełną szerokość. Cała strona jest przez to **jednokolumnowa**: hero, nagłówek „Products", każda etykieta i każda karta mają tę samą lewą krawędź i tę samą szerokość. Wcześniej numer stał w stałej kolumnie 180px obok karty, przez co karty były węższe niż wszystko nad nimi. Karta: `--pf-surface-card`, `var(--pf-hairline)`, radius 24, padding 32, `gap: 32` — kolejno znacznik 32×32 (radius 8, tło `--pf-accent-500`, ikona lucide 19px `strokeWidth 1.75` na biało), blok tytułu (h3 `.pf-h4` + chipy + intro), blok zakresu (h4 `.pf-h4` + `.pf-role-list`), rząd obrazków i podpis `.pf-caption` w `--pf-text-muted`.
