...
- **Dwa źródła przeglądarki, i to nie jest nadmiarowe.** Lokalnie leci Chrome pobrany przez puppeteer do `node_modules/.cache/puppeteer` (`.puppeteerrc.cjs` — `~/.cache` nie przechodzi między buildami na Vercelu). Na Vercelu leci `@sparticuz/chromium`, bo obraz builda **nie ma bibliotek systemowych**, z którymi linkuje się Chrome for Testing — pobrana binarka startuje lokalnie i umiera na deployu z `libnspr4.so: cannot open shared object file` (kod 127). Ta paczka wozi build pod Amazon Linux 2023 razem z tymi bibliotekami (`bin/al2023.tar.br`) i ustawia na nie `LD_LIBRARY_PATH`. Rozgałęzienie idzie po `process.env.VERCEL`; tam `.puppeteerrc.cjs` ustawia też `skipDownload`, żeby nie ciągnąć 150 MB binarki, która i tak nie wstanie.
- **`headless: "shell"` jest wpisane na sztywno.** `@sparticuz/chromium` wozi `chrome-headless-shell`, a swój eksport `headless` wyrzucił w v149 — `chromium.headless` zwraca dziś `undefined`, czyli puppeteer odpaliłby tę binarkę z `--headless=new`. Nie zamieniaj tego z powrotem na `chromium.headless`.
- **`chromium.setGraphicsMode = true`** zostaje: przy `false` paczka dokłada `--disable-webgl` i nie rozpakowuje SwiftShadera, a hero maluje się przez WebGL (ogl) i wywaliłby się w trakcie renderu, który próbujemy złapać.
